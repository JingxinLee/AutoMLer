...
def normalize_image(examples):
    ...
    def apply_transform(examples):
        images = [
            transform(Image.open(io.BytesIO(image))) for image in examples["image"]
        ]
        return {"image": images}
...
def augment_text(aug_text):
    aug = [
        ...
        textaugs.replace_fun_fonts(
            aug_text, vary_fonts=True, granularity="word", metadata=[]
        ),
...
def augment_image(aug_image):
    aug = [
        ...
        imaugs.change_aspect_ratio(
            aug_image, ratio=random.uniform(0.8, 1.5)
        ),  # 改变图像宽高比
        imaugs.color_jitter(
            aug_image,
            brightness_factor=random.uniform(0.8, 1.5),
            contrast_factor=random.uniform(0.8, 1.5),
            saturation_factor=random.uniform(0.8, 1.5),
        ),  # 颜色晃动
        imaugs.crop(
            aug_image,
            x1=random.uniform(0, 0.1),
            y1=random.uniform(0, 0.1),
            x2=random.uniform(0.9, 1),
            y2=random.uniform(0.9, 1),
        ),  # 随机裁剪
        ...
        imaugs.shuffle_pixels(
            aug_image, factor=random.uniform(0, 0.1)
        ),  # 随机像素比任意化
...
def augment_audio(aug_audio):
    ...
    aug = [
        ...
        )(input_audio_arr, sample_rate=sr, metadata=[]),
    ]
    return random.choice(aug)
...
def augment_video(aug_video):
    ...
    aug = [
        ...
        vidaugs.loop(
            video_path=aug_video, output_path=output_path, num_loops=1, metadata=[]
        ),
        vidaugs.InsertInBackground()(
            video_path=aug_video, output_path=output_path, metadata=[]
        ),
        ...
        )(video_path=aug_video, output_path=output_path),
    ]
    return random.choice(aug)
