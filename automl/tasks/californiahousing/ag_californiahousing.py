...
def get_parser():
    ...
    parser.add_argument(
        "--automm-mode",
        choices=["ft-transformer", "mlp"],
        default="ft-transformer",
        help="Fusion model in AutoMM.",
    )
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/home/ddp/nlp/github/paper/mypaper_code/automl/data/california-house-prices",
    )
    parser.add_argument("--seed", type=int, default=123)
    parser.add_argument(
        "--exp_path",
        default="/home/ddp/nlp/github/paper/mypaper_code/automl/data/california-house-prices/submission",
    )
...
def train(args):
    ...
    if args.mode == "single":
        predictor = MultiModalPredictor(
            eval_metric=eval_metric, label=label_column, path=args.exp_path
        )
        predictor.fit(train_df, hyperparameters=automm_hyperparameters, seed=args.seed)
    elif (
        args.mode == "weighted"
        or args.mode == "stack5"
        or args.mode == "single_bag5"
        or args.mode == "single_bag4"
    ):
